...
<div class="max-w-3xl mx-auto">
  ...
  <div class="mb-6 flex items-center justify-between">
    <div>
      <h1 class="text-xl font-bold text-slate-900">Edit Layanan</h1>
      <p class="text-xs text-slate-500">Perbarui detail layanan {{ $service->title }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.services.update', $service) }}" enctype="multipart/form-data" class="space-y-5">
      ...
      <div>
        <div class="flex items-center justify-between mb-1.5">
          ...
          <span class="text-[11px] text-slate-400">Dapat disesuaikan</span>
        </div>
            ...
            @foreach($service->features as $feat)
              <div class="flex items-center space-x-2">
                <input type="text" name="features[]" value="{{ $feat }}"
                       class="flex-1 px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 focus:bg-white focus:border-[#21C9A4] focus:ring-2 focus:ring-[#21C9A4]/20 outline-hidden">
      ...
      <div>
        <label class="block text-xs font-semibold text-slate-700 mb-1.5">Gambar / Banner Layanan</label>
        
        @if($service->image)
          <div class="flex items-center space-x-4 mb-3 p-3 bg-slate-50 border border-slate-200 rounded-xl">
            <img src="{{ Storage::url($service->image) }}" alt="Preview" class="w-16 h-16 object-cover rounded-lg border border-slate-200">
            <div>
              <span class="text-xs font-semibold text-slate-700 block">Gambar Terpasang</span>
              <span class="text-[11px] text-slate-400">Pilih berkas di bawah jika ingin mengunggah gambar pengganti.</span>
            </div>
          </div>
        @endif
...
<script>
  ...
  function createFeatureInput(value = '') {
    ...
    div.innerHTML = `
      <input type="text" name="features[]" value="${value}" placeholder="Poin fitur baru..."
