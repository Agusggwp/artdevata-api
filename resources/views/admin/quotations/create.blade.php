...
@section('title', 'Buat Quotation Penawaran')
...
<div class="max-w-5xl mx-auto space-y-6">
  ...
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-xl font-bold text-slate-800">Buat Quotation Penawaran Baru</h1>
      <p class="text-xs text-slate-500">Rincian item penawaran harga untuk calon klien ARTDEVATA.</p>
    </div>
  ...
  <form method="POST" action="{{ route('admin.quotations.store') }}" id="quotation-form" class="space-y-6">
    @csrf

    <!-- Quotation Header Info Card -->
    <div class="bg-white rounded-2xl p-6 shadow-card border border-slate-100 space-y-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <input type="text" name="quotation_number" value="{{ old('quotation_number', $autoQuotationNumber) }}" required class="w-full text-xs font-bold text-[#14433B] rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">
        </div>
        ...
        <div>
          ...
          <input type="date" name="issue_date" value="{{ old('issue_date', date('Y-m-d')) }}" required class="w-full text-xs rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">
        </div>

        <div>
          <label class="block text-xs font-bold text-slate-700 mb-1">Berlaku Sampai *</label>
          <input type="date" name="valid_until" value="{{ old('valid_until', date('Y-m-d', strtotime('+14 days'))) }}" required class="w-full text-xs rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">
        </div>
        ...
        <table class="w-full text-left border-collapse" id="items-table">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-100 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
              <th class="py-2.5 px-3">Layanan (Snapshot)</th>
      ...
      <div class="flex flex-col md:flex-row md:items-start justify-between gap-6 pt-4 border-t border-slate-100">
        <div class="space-y-4 flex-1">
          <div>
            ...
            <textarea name="terms" rows="3" class="w-full text-xs rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">1. Pembayaran DP 50% saat persetujuan.
2. Pelunasan 50% setelah pekerjaan selesai.
3. Penawaran berlaku sesuai tanggal validitas.</textarea>
          </div>
          <div>
            <label class="block text-xs font-bold text-slate-700 mb-1">Catatan Tambahan untuk Klien</label>
            <textarea name="notes" rows="2" placeholder="Catatan opsional..." class="w-full text-xs rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">{{ old('notes') }}</textarea>
          </div>
        </div>
        ...
        <div class="w-full md:w-80 bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3 text-xs">
          ...
          <div class="flex justify-between items-center text-slate-600">
            ...
            <input type="number" step="0.01" name="discount" id="global-discount" value="{{ old('discount', 0) }}" oninput="calculateTotals()" class="w-32 text-xs text-right rounded-lg border-slate-200 py-1">
          </div>
          <div class="flex justify-between items-center text-slate-600">
            <span>Pajak PPN (Rp):</span>
            <input type="number" step="0.01" name="tax" id="global-tax" value="{{ old('tax', 0) }}" oninput="calculateTotals()" class="w-32 text-xs text-right rounded-lg border-slate-200 py-1">
          </div>
    ...
    <div class="flex items-center justify-end space-x-3">
      ...
      <button type="submit" class="px-5 py-2.5 rounded-xl bg-[#14433B] hover:bg-[#0B443C] text-white text-xs font-semibold shadow-sm transition-all">Simpan & Terbitkan Quotation</button>
    </div>
...
<script>
  ...
  function addItemRow(serviceId = '', description = '', qty = 1, unit = 'unit', price = 0, discount = 0) {
    ...
    tr.innerHTML = `
      ...
      <td class="py-2 px-2">
        <input type="text" name="items[${itemIndex}][description]" value="${description}" required placeholder="Deskripsi pekerjaan..." class="w-full text-xs rounded-lg border-slate-200 py-1">
      </td>
      <td class="py-2 px-2">
      ...
      <td class="py-2 px-2">
        <input type="text" name="items[${itemIndex}][unit]" value="${unit}" required placeholder="unit/paket" class="w-full text-xs rounded-lg border-slate-200 py-1">
      </td>
      <td class="py-2 px-2">
  ...
  document.addEventListener('DOMContentLoaded', () => {
    addItemRow('', 'Website Development & Design', 1, 'paket', 5000000, 0);
  });
</script>
@endsection
