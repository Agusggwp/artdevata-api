...
<div class="max-w-4xl mx-auto space-y-6">
  ...
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-xl font-bold text-slate-800">Tambah Lead Prospek Baru</h1>
      <p class="text-xs text-slate-500">Masukkan detail informasi prospek baru ke dalam sistem CRM.</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.leads.store') }}" class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="name" value="{{ old('name') }}" required class="w-full text-xs rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">
        </div>

        <div>
          <label class="block text-xs font-bold text-slate-700 mb-1">Nama Perusahaan / Instansi</label>
          <input type="text" name="company_name" value="{{ old('company_name') }}" placeholder="PT Example Indonesia" class="w-full text-xs rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">
        </div>

        <div>
          <label class="block text-xs font-bold text-slate-700 mb-1">Email</label>
          <input type="email" name="email" value="{{ old('email') }}" class="w-full text-xs rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">
        </div>

        <div>
          <label class="block text-xs font-bold text-slate-700 mb-1">Nomor Telepon / WhatsApp</label>
          <input type="text" name="phone" value="{{ old('phone') }}" placeholder="08123456789" class="w-full text-xs rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">
        </div>
        ...
        <div>
          ...
          <input type="text" name="service_interest" value="{{ old('service_interest') }}" placeholder="Web Development, Network Setup, CCTV..." class="w-full text-xs rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">
        </div>

        <div>
          <label class="block text-xs font-bold text-slate-700 mb-1">Estimasi Budget (Rp)</label>
          <input type="number" step="0.01" name="estimated_budget" value="{{ old('estimated_budget', 0) }}" class="w-full text-xs rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">
        </div>
        ...
        <div>
          ...
          <input type="datetime-local" name="next_follow_up_at" value="{{ old('next_follow_up_at') }}" class="w-full text-xs rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">
        </div>
      </div>

      <div>
        <label class="block text-xs font-bold text-slate-700 mb-1">Alamat Prospek</label>
        <textarea name="address" rows="2" class="w-full text-xs rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">{{ old('address') }}</textarea>
      </div>

      <div>
        <label class="block text-xs font-bold text-slate-700 mb-1">Catatan Tambahan</label>
        <textarea name="notes" rows="3" placeholder="Catatan diskusi, kebutuhan spesifik..." class="w-full text-xs rounded-xl border-slate-200 focus:border-[#14433B] focus:ring-[#14433B]">{{ old('notes') }}</textarea>
      </div>
      ...
      <div class="flex items-center justify-end space-x-3 pt-4 border-t border-slate-100">
        ...
        <button type="submit" class="px-5 py-2.5 rounded-xl bg-[#14433B] hover:bg-[#0B443C] text-white text-xs font-semibold shadow-sm transition-all">Simpan Lead Prospek</button>
      </div>
    </form>
  </div>
