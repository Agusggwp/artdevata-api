...
<div class="max-w-4xl mx-auto">
  ...
  <div class="mb-6 flex items-center justify-between">
    <div>
      <h1 class="text-xl font-bold text-slate-900">Edit Proyek</h1>
      <p class="text-xs text-slate-500">Perbarui informasi, status, dan tim proyek {{ $project->name }}</p>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div>
          ...
          <select 
            ...
            required
          >
            <option value="pending" {{ old('status', $project->status) === 'pending' ? 'selected' : '' }}>Tertunda (Pending)</option>
      ...
      <div class="pt-4 border-t border-slate-100">
        <div class="flex items-center justify-between mb-3">
          <div>
            ...
            <p class="text-[11px] text-slate-400">Atur anggota tim yang bertugas pada proyek ini</p>
          </div>
              ...
              <div class="flex-1">
                <select name="team_members[]" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 focus:bg-white focus:border-[#21C9A4] outline-hidden">
                  ...
                  @foreach($admins as $admin)
                    <option value="{{ $admin->id }}" {{ $admin->id === $member->id ? 'selected' : '' }}>{{ $admin->name }} ({{ $admin->email }})</option>
                  @endforeach
                </select>
              </div>
              <div class="flex-1">
