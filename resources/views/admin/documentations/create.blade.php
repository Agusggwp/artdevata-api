...
@section('title', 'Tambah Dokumentasi Perusahaan')
...
<div class="flex items-center justify-between mb-6">
  <div>
    <h1 class="text-xl font-bold text-slate-900">Tambah Dokumentasi Baru</h1>
    <p class="text-xs text-slate-700 font-medium">Unggah foto kegiatan, instalasi, kalibrasi, atau hasil kerja perusahaan</p>
  </div>
...
<div class="bg-white rounded-2xl p-6 sm:p-8 shadow-card border border-slate-200 max-w-3xl">
  ...
  <form method="POST" action="{{ route('admin.documentations.store') }}" enctype="multipart/form-data" class="space-y-6">
    @csrf
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      ...
      <div>
        ...
        >
          ...
          <option value="inactive" {{ old('status') == 'inactive' ? 'selected' : '' }}>Draft (Disembunyikan)</option>
        </select>
      </div>
    ...
    <div>
      <label for="image" class="block text-xs font-bold text-slate-800 mb-1.5">Unggah Foto Dokumentasi</label>
      
      <div class="flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
        <!-- Preview Box -->
        <div id="image-preview-container" class="w-32 h-32 rounded-xl bg-slate-100 border-2 border-dashed border-slate-300 flex flex-col items-center justify-center text-slate-400 overflow-hidden shrink-0">
          <i class="fa-solid fa-cloud-arrow-up text-2xl mb-1"></i>
          <span class="text-[10px] font-medium">Preview Foto</span>
        </div>
        ...
        <div class="flex-1 w-full">
          ...
          >
          <p class="text-[11px] text-slate-500 mt-1.5">Format: JPG, PNG, WEBP, GIF (Maks. 4MB). Rekomendasi rasio gambar tegak (3:4) atau persegi (1:1).</p>
        </div>
      </div>
      @error('image')
        <p class="text-rose-600 text-[11px] font-semibold mt-1">{{ $message }}</p>
